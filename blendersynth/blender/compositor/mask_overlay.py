...
class MaskOverlay(CompositorNodeGroup):
	def __init__(self, name, node_tree, index=0, use_antialiasing=True, dtype='Color'):
		...
		self.group.links.new(self.input_node.outputs['IndexOB'], self.mask_node.inputs['ID value'])
		self.group.links.new(self.input_node.outputs['Image'], self.mix_node.inputs[2])
		self.group.links.new(self.mask_node.outputs['Alpha'], self.mix_node.inputs['Fac'])
		self.group.links.new(self.mask_node.outputs['Alpha'], self.alpha_node.inputs['Alpha'])
		self.group.links.new(self.mix_node.outputs['Image'], self.alpha_node.inputs['Image'])
		self.group.links.new(self.alpha_node.outputs['Image'], self.output_node.inputs['Image'])
