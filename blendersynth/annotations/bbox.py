...
def _bounding_box(object: Mesh, camera: bpy.types.Camera = None,
...
def _bounding_boxes(objects: List[Mesh], camera: bpy.types.Camera = None,
				 scene:bpy.types.Scene=None,
	...
	return [_bounding_box(obj, camera=camera, scene=scene, return_fmt=return_fmt,
						   normalized=normalized, invert_y=invert_y) for obj in objects]

def bounding_boxes(objects: List[Mesh], camera: Union[Camera, List[Camera]] = None,
				   scene:bpy.types.Scene=None, return_fmt:str='x1y1x2y2', normalized:bool=False, invert_y:bool=True) -> AnnotationHandler:
	"""
	...
	for cam in camera:
		return_dict[cam.name] = _bounding_boxes(objects, camera=cam, scene=scene,
												return_fmt=return_fmt, normalized=normalized, invert_y=invert_y)
